...
class LstmPage extends StatefulWidget {
  const LstmPage({Key? key}) : super(key: key);
...
class _LstmPageState extends State<LstmPage> {
  ...
  Future<Map<String, dynamic>> predictLstm(List<int> fileBytes) async {
    var uri = Uri.parse('http://127.0.0.1:8000/predict/lstm');
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to predict with LSTM: $errorData');
    }
  }
  ...
  Future<void> getFile() async {
    ...
    if (result != null) {
      ...
      if (fileBytes != null) {
        ...
        try {
          var prediction = await predictLstm(fileBytes);
          setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('LSTM - Time Series Prediction'),
      ),
